...
//        User userpass = User.get(session.user.id)
        User u = User.get(session.user.id)

//        render session.user.id

        if(u){
            if(params.renewpass == params.newpass)  {
                if(u.password.equals(params.currpass)){
                    u.password=params.newpass
                    u.save(flush: true)
                    flash.message = "Your Password has change successfully"
                    redirect(controller: 'user' ,action: 'dashBoard')
                }else{
                    flash.message = "You Current Password is not correct"
                    redirect(controller: 'user',action: 'reset')
                }
            }else{
                flash.message = "Both Password does not match"
                redirect(controller: 'user',action: 'reset')
            }
...
//        render ("your password has reset successfully ${u}")
    }
}
